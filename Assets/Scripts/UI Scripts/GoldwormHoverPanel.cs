...
public sealed class GoldwormHoverPanel : MonoBehaviour, IHoverInfoUI
{
    [Header("Spawn")]
    ...
    [Header("Content")]
    [SerializeField] private string title = "Goldworm";
    ...
    private void SpawnPanel()
    {
        ...
        {
            Debug.LogError($"{nameof(GoldwormHoverPanel)}: screenSpacePanelPrefab not assigned.", this);
            enabled = false;
            return;
        }
        ...
        if (rootGO == null)
        {
            Debug.LogError($"{nameof(GoldwormHoverPanel)}: could not find WorldUIRoot with tag '{worldUIRootTag}'.", this);
            enabled = false;
            return;
        }
        ...
        if (canvas == null)
        {
            Debug.LogError($"{nameof(GoldwormHoverPanel)}: WorldUIRoot has no Canvas.", this);
            enabled = false;
            return;
        }
        ...
        if (view == null)
        {
            Debug.LogError($"{nameof(GoldwormHoverPanel)}: HoverPanelView missing on panel prefab.", this);
            enabled = false;
            return;
        }
